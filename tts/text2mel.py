...
from tts.config import FLAGS, DurationInput
from tts.nat_model import AcousticModel, DurationModel
...
def predict_duration(tokens):
    ...
    with open("src/duration_latest_ckpt.pickle", "rb") as f:
        dic = pickle.load(f)
    x = DurationInput(
...
def text2mel(
        text: str, silence_duration: float = -1.0
):
    tokens = text2tokens(text, "src/lexicon.txt")
